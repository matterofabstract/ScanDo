...
const photoLibrary = require('os').homedir() + '/Downloads/SantaPhotos';
const incomingDir =  photoLibrary + '/incoming';
...
if (fs.existsSync(incomingDir)) {
  ...
  console.log('Incoming Photo Directory (' + incomingDir + ') doesn\'t exist. Creating...');
...
let watcher = chokidar.watch(
   incomingDir, { ignored: /(^|[/\\])\../, persistent: true }
);
...
watcher
  .on('add', path => {
    ...
    if (SANTA_CODE) {
      fs.rename(path, photoLibrary + '/' + process.env.SANTA_CODE + '/' + filename, (err) => {
        if (err) throw err;
